...
class CanvasRenderer extends DisplayObjectRenderer
{
	...
	**/
	...
	#if (js && html5)
	// group canvases (object, backdrop copy) per nesting level, shared by every renderer
	...
	**/
	...
	/**
		INVERT: a white silhouette of the object (source-in on the group canvas) is
		differenced with a copy of the backdrop, (1 - a) * x + a * (1 - x) per
		channel, and the copy goes back with source-atop so the backdrop keeps
		its own alpha (nothing appears where it was transparent, as in Flash).
	**/
	...
	/**
		SUBTRACT: canvas has no subtract operation. On a copy of the backdrop x,
		max(0, x - p) with the premultiplied object p is built as
		invert(invert(x) + p): a white difference inverts, lighter adds the
		object, a white difference inverts back. The copy goes back with
		source-atop so the backdrop keeps its own alpha.
	**/
